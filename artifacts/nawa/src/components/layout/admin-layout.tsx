        ...
        <header className="h-16 bg-white border-b border-border flex items-center justify-between px-6 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm text-muted-foreground">
              <span className="font-semibold text-foreground">{language === "ar" ? "بوابة الإدارة" : "Admin Portal"}</span>
              <ChevronRight className="w-4 h-4 rtl:-scale-x-100" />
              <span className="capitalize">{location.split("/").pop()?.replace("-", " ") || "dashboard"}</span>
            </div>
          </div>
