...
export default function AdminProjects() {
  ...
  useEffect(() => {
    document.title = language === "ar" ? "إدارة المشاريع | نوى العقارية" : "Manage Projects | Nawa Real Estate Platform";
  }, [language]);
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="imageUrl" render={({ field }) => (<FormItem className="col-span-2"><FormLabel>{language === "ar" ? "صورة المشروع" : "Project Image"}</FormLabel><FormControl><ImageUpload value={field.value} onChange={field.onChange} aspectRatio="wide" /></FormControl></FormItem>)} />
              </div>
              {/* AI auto-generate descriptions from titles */}
              <div className="flex flex-wrap gap-2 items-center bg-gradient-to-r from-primary/5 to-secondary/5 border border-primary/20 rounded-lg p-3">
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const ar = await aiTextAction("expand", t, "وصف عقاري احترافي قصير لمشروع باسم: " + t);
                      const en = await aiTextAction("translate_en", ar);
                      form.setValue("descriptionAr", ar);
                      form.setValue("description", en);
                      toast({ title: language === "ar" ? "تم التوليد بالذكاء الاصطناعي ✨" : "Generated by AI ✨" });
                    } catch (e: any) {
                ...
                >
                  ...
                  {language === "ar" ? "ولّد الأوصاف تلقائياً" : "Auto-generate descriptions"}
                </Button>
              </div>
